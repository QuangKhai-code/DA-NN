...
# Dataset settings
source_dataset_name = 'private'
target_dataset_name = 'web'
...
wandb.login()
# Initialize wandb
wandb.init(project="domain-adaptation", name="custom-dataset")
...
# Define the datasets
source_dataset_name_train = os.path.join(source_image_root, 'train')
source_dataset_name_val = os.path.join(source_image_root, 'val')


target_dataset_name_train = os.path.join(target_image_root, 'train')
target_dataset_name_val = os.path.join(target_image_root, 'val')
...
my_net = CNNModel(num_classes=12)
...
def calculate_metrics(loader, dataset_name):
    test_net = CNNModel(num_classes=12)
